...
async function loadData() {
  try {
    ...
    const [cryptoResponse, sentimentResponse, correlationResponse] =
      await Promise.all([
        fetch(`resources/data/crypto-prices.json?t=${timestamp}`),
        fetch(`resources/data/sentiment-data.json?t=${timestamp}`),
        fetch(`resources/data/correlation-data.json?t=${timestamp}`),
      ]);
...
// Update sentiment gauge with current data
function updateSentimentGauge() {
  if (!sentimentGauge) return;
  
  // Get sentiment from either sentimentData or cryptoData
  let sentimentValue = 0.5;
  
  if (sentimentData.sentimentOverview && sentimentData.sentimentOverview.overallSentiment !== undefined) {
    sentimentValue = sentimentData.sentimentOverview.overallSentiment;
  } else if (cryptoData.marketOverview && cryptoData.marketOverview.socialSentiment !== undefined) {
    sentimentValue = cryptoData.marketOverview.socialSentiment;
  }
...
function updatePriceChart() {
  ...
  // Use real sparkline data from the API
  const sparkline = crypto.sparkline || [];
  const data = [];
  const now = new Date();
  
  // If we have real sparkline data, use it
  if (sparkline.length > 0) {
    const intervalMs = getTimeframeInterval();
    for (let i = 0; i < sparkline.length; i++) {
      const time = new Date(now.getTime() - (sparkline.length - 1 - i) * intervalMs);
      data.push([time.toISOString(), sparkline[i].toFixed(2)]);
    }
  } else {
    // Fallback: generate data based on current price if no sparkline available
...
// Get interval in milliseconds based on current timeframe
function getTimeframeInterval() {
  switch (currentTimeframe) {
    case '1h': return 60000;      // 1 minute
    case '4h': return 240000;     // 4 minutes
    case '24h': return 1440000;   // 24 minutes
    case '7d': return 10080000;   // 7 days in chunks
    case '30d': return 43200000;  // 30 days in chunks
