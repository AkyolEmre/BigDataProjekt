...
class RealtimeCryptoFetcher:
    def __init__(self, update_interval=10):
        ...
        self.sentiment_file = os.path.join(self.output_dir, 'sentiment-data.json')
    ...
    def fetch_data(self):
        ...
        sentiment_data = self.generate_sentiment_data(cryptocurrencies, avg_sentiment)
    ...
    def generate_sentiment_data(self, cryptocurrencies, avg_sentiment):
        """Generate sentiment analysis data based on crypto data"""
        crypto_sentiments = []
        for crypto in cryptocurrencies:
            crypto_sentiments.append({
                ...
                "buzzVolume": crypto["buzzVolume"],
                "trendDirection": "up" if crypto["change24h"] > 0 else "down",
                "confidence": round(0.6 + abs(crypto["socialSentiment"]) * 0.3, 2)
            })
        
        return {
            "sentimentOverview": {
                "overallSentiment": round(avg_sentiment, 2),
                "bullishPercentage": round((avg_sentiment + 1) * 50, 1),
                "bearishPercentage": round((1 - avg_sentiment) * 50, 1),
                "neutralPercentage": round(max(0, 20 - abs(avg_sentiment) * 20), 1),
                "totalMentions": sum(c["buzzVolume"] for c in cryptocurrencies),
                "timestamp": datetime.now().isoformat()
            },
            "cryptoSentiments": crypto_sentiments,
            "sources": {
                "twitter": round(40 + np.random.uniform(-5, 5), 1),
                "reddit": round(30 + np.random.uniform(-5, 5), 1),
                "news": round(20 + np.random.uniform(-3, 3), 1),
                "telegram": round(10 + np.random.uniform(-2, 2), 1)
            },
            "lastUpdate": datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        }
