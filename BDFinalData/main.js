...
async function loadData() {
    try {
        const [cryptoResponse, sentimentResponse, correlationResponse] = await Promise.all([
...
function loadFallbackData() {
    cryptoData = {
        cryptocurrencies: [
            {
                symbol: "BTC", name: "Bitcoin", price: 98750.32, change24h: 2.34,
                volume24h: 28500000000, marketCap: 1950000000000, volatility: 0.0234,
                sparkline: [98123, 98345, 98750, 98567, 98923, 98750, 99123, 98750, 98543, 98750],
                socialSentiment: 0.67, buzzVolume: 89432
            },
            {
                symbol: "ETH", name: "Ethereum", price: 3456.78, change24h: -1.23,
                volume24h: 15600000000, marketCap: 415000000000, volatility: 0.0312,
                sparkline: [3456, 3423, 3456, 3489, 3456, 3434, 3456, 3478, 3456, 3456],
                socialSentiment: 0.45, buzzVolume: 56789
            }
        ],
        marketOverview: {
...
function initializeSentimentGauge() {
    ...
    const option = {
        ...
        series: [{
            ...
            data: [{
                value: sentimentData.sentimentOverview ? sentimentData.sentimentOverview.overallSentiment : 0.67,
...
function populateCryptoCards() {
    ...
    cryptoData.cryptocurrencies.forEach(crypto => {
        ...
        card.className = 'bg-gray-900 rounded-xl p-6 card-hover border border-gray-700 cursor-pointer';
        ...
        card.innerHTML = `
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center space-x-3">
                    <div class="w-10 h-10 bg-gradient-to-r from-blue-500 to-emerald-500 rounded-full flex items-center justify-center">
                        <span class="text-white font-bold text-sm">${crypto.symbol}</span>
                    </div>
                    <div>
                ...
                <div class="text-right">
                    ...
                    <p class="${changeColor} text-sm">${changeIcon} ${Math.abs(crypto.change24h)}%</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <p class="font-semibold">$${(crypto.marketCap / 1000000000).toFixed(2)}B</p>
                </div>
                <div>
                    <p class="text-gray-400 text-sm">24h Volume</p>
                    <p class="font-semibold">$${(crypto.volume24h / 1000000000).toFixed(2)}B</p>
                </div>
            </div>
            ...
            <div class="mb-4">
                <div class="flex justify-between items-center mb-2">
                    ...
                    <span class="text-sm">${(crypto.volatility * 100).toFixed(2)}%</span>
                </div>
                <div class="w-full bg-gray-700 rounded-full h-2">
                    <div class="bg-gradient-to-r from-blue-400 to-emerald-400 h-2 rounded-full" style="width: ${Math.min(crypto.volatility * 1000, 100)}%"></div>
                </div>
            </div>
            ...
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="font-semibold ${crypto.socialSentiment >= 0 ? 'text-emerald-400' : 'text-red-400'}">
                        ${crypto.socialSentiment >= 0 ? '+' : ''}${crypto.socialSentiment.toFixed(2)}
                    </p>
                </div>
                <div>
...
function updateMarketOverview() {
    ...
    document.getElementById('total-market-cap').textContent = 
        `$${(overview.totalMarketCap / 1000000000000).toFixed(2)}T`;
    document.getElementById('total-volume').textContent = 
        `$${(overview.totalVolume / 1000000000).toFixed(1)}B`;
    document.getElementById('btc-dominance').textContent = `${overview.btcDominance}%`;
...
function startRealTimeUpdates() {
    // Update all data every 2 minutes (120 seconds) with real data
    setInterval(() => {
